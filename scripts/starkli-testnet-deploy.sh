...
stdout=$(starkli deploy --watch 0x0148172b369b7eb959f1292f3fe2132a82fbe2c043f429299a31b12bb8caa21a) 
nogame=$(echo "$stdout" | grep -o '0x[0-9a-fA-F]\+')
echo 'NOGAME::'${nogame} > deployed_contracts.txt

echo 'deploying NFT...'
stdout=$(starkli deploy --watch 0x001feb052b2ea7b1f744692306346ebc70ac966fd3a55774f9e576ca435bd959 0x4e6f47616d65207465737420506c616e6574 0x4e47744e4654 $nogame $receiver)
