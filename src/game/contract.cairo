...
mod Game {
    ...
    // Segregated interface implementations for better separation of concerns
    use nogame::game::interfaces::{
        IContractRegistry, IResourceManager, ITokenProvider, IUniverseConfig,
    };
    ...
    /// Universe configuration implementation
    #[abi(embed_v0)]
    ...
    /// Administrative interface for one-time setup functions
    #[starknet::interface]
    trait IGameAdmin<TState> {
        fn initialize(
            ref self: TState,
            colony: ContractAddress,
            compound: ContractAddress,
    ...
    #[abi(embed_v0)]
    impl GameAdminImpl of IGameAdmin<ContractState> {
        fn initialize(
